			...
			<view v-for="item in orderList" :key="item.id" class="box" style="margin-bottom: 10rpx;"
				...
				<view style="display: flex; align-items: center;">
					...
					<view style="flex: 1; text-align: right;">
						...
						<view style="display: inline-block;margin-right: 5px;" v-if="item.tid !== null" @click.native.stop="preventParentClick">
											<uni-icons text="举报" type="info" size="20" color="#888" @click="report(item)" ></uni-icons>
						...
						<uni-tag text="评价订单" type="success" size="small" v-if="item.status === '待评价'" @click.native.stop="goComment(item.id)"></uni-tag>
	...
	export default {
		data() {
			return {
				...
				orderId: 0,
				selectedReasonIndex: 0, // 默认选中第一个举报理由
				  reasons: [ // 举报理由列表  
				        { name: '虚假信息', value: 'false_info' },  
				        { name: '违规内容', value: 'violation_content' },  
		...
		methods: {
			...
			report(r){
				this.tid=r.id
				uni.navigateTo({
				 	url: '/pages/report/report?tid='+this.tid
				 })
