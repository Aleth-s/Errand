  ...
  <view class="forum">
    <view v-for="(r, index) in info" :key="index" class="post">
      <view class="post-header">
        ...
        <view class="post-weight">重量:{{ r.otherinfo }}</view>
      ...
      <view class="post-author">
        ...
        <view class="post-price">{{ r.price }}¥</view> <!-- 将价格信息放置在作者用户名右侧 -->
      </view>
	  <view class="line">
	  ...
	  <button class="want-trade-button" @click="wantToTrade(r)">想要交易</button>
    </view>
...
export default {
  ...
  methods: {
    load() {
      this.$request.get('/transaction/selectAll').then(res => {
        this.info = res.data || [];
      });
    },
    submitPost() {
			uni.navigateTo({
        url: '/pages/trans_post/trans_post'
      });
    },
	wantToTrade(r){
		this.tid=r.id
